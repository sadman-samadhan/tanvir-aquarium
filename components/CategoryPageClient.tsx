...
import { ShoppingBag, Search, Eye, Filter, ArrowLeft } from 'lucide-react'
...
export default function CategoryPageClient({ category, allCategories, initialProducts }: CategoryPageClientProps) {
  ...
  // Filter products based on search and category ID (including subcategories/variations)
  const filteredProducts = products.filter((product) => {
    ...
    const matchesCategory = productCatIds.includes(category.id)
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50">
      ...
      <section className="relative overflow-hidden bg-slate-900 text-white py-16 sm:py-24">
        <div className="absolute inset-0 z-0 opacity-30 bg-[url('https://images.unsplash.com/photo-1544551763-46a013bb70d5?q=80&w=1600')] bg-cover bg-center"></div>
        ...
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl">
            <Link
              href="/"
              className="inline-flex items-center gap-1 text-xs font-bold text-emerald-400 hover:text-emerald-300 transition-colors uppercase tracking-wider mb-4"
            >
              <ArrowLeft className="h-3.5 w-3.5" /> Back to Storefront
            </Link>
            <h1 className="text-3xl font-extrabold tracking-tight sm:text-5xl text-white uppercase">
              {category.name}
            </h1>
            <p className="mt-4 text-base leading-relaxed text-slate-300">
              {category.description || `Browse our premium range of ${category.name} products.`}
            </p>
          </div>
        </div>
      </section>
      ...
      <main className="flex-grow mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 pb-8 border-b border-slate-200">
          <div>
            <h2 className="text-xl font-bold tracking-tight text-slate-950">Category Catalog</h2>
        ...
        <div className="flex flex-wrap gap-2 py-8 overflow-x-auto">
          <Link
            href="/"
            className="rounded-full px-4 py-2 text-xs font-semibold tracking-wide transition-all border bg-white text-slate-700 border-slate-200 hover:border-slate-300"
          >
            All Products
          </Link>
          {categories.map((cat) => (
        ...
        {filteredProducts.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-lg border border-dashed border-slate-300">
            ...
            <p className="mt-1 text-sm text-slate-500">No items available under this category matched your search.</p>
          </div>
        ) : (
